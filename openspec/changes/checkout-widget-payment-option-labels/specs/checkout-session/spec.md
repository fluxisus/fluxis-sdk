...
`CheckoutWidget` SHALL render `PendingScreen` (not a standalone full-screen component) when `session.status === 'selecting_asset'`, identical to the `pending` case, showing the QR (`FluxisQrCode`, keyed on `session.recipient_address`) immediately if present. `AssetSelectionScreen`, listing the options in `session.payment_options` (an array of `{unique_asset_id, symbol, network}` objects), SHALL render inside `PendingScreen`'s manual-transfer accordion instead — reachable by expanding "Transferencia manual" — rather than as a top-level full-screen picker, and SHALL render each option's `symbol` and `network` directly rather than deriving them from `unique_asset_id`. `CheckoutWidgetProps` SHALL continue to accept an optional `onSelectAsset: (assetId: string) => Promise<void>` callback, passed through to wherever `AssetSelectionScreen` renders. `AssetSelectionScreen` SHALL NOT perform its own network request — submitting the selection to the backend remains the responsibility of the code that supplies `onSelectAsset`.
...
- **THEN** the accordion shows `AssetSelectionScreen`'s option list, one button per `session.payment_options` entry, each labeled with that option's `symbol` (primary) and `network` (secondary) — not amount/address details

#### Scenario: Two options share a network
- **WHEN** `session.payment_options` contains two entries with the same `network` but different `symbol` (e.g. USDT and USDC, both Polygon)
- **THEN** the two buttons render with distinct, legible labels (their symbols), not identical text

#### Scenario: Shopper selects an asset
- **WHEN** the shopper clicks one of the rendered options inside the manual-transfer accordion
- **THEN** `onSelectAsset` SHALL be called with that option's `unique_asset_id`, and the picker SHALL show a loading state until the returned promise settles
